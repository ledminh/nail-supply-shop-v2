...
function ProductList({ products, type }: Props) {
  ...
  const [quantities, setQuantities] = useState<Record<string, number>>(
    toQuantites(products)
  ); // product id -> quantity

  const [selectedProductIDs, setSelectedProductIDs] = useState<
    Record<string, string>
  >(toSelectedProductIDs(products)); // product group id -> product id
...
function isProductGroup(
  product: Product | ProductGroup
): product is ProductGroup {
...
function getProductItemCPN(
  ...
  setSelectedProductIDs: React.Dispatch<
  ...
  const ProductItemCPN = (props: Product | ProductGroup) => {
    return (
      <CartContext.Consumer>
        {({ addToCart }) => {
          return isProductGroup(props) ? (
            <ProductGroupBlock
              {...props}
              addToCart={addToCart}
              setInitQuantity={setQuantity}
              initQuantities={quantities}
              initSelectedProductID={selectedProductIDs[props.id]}
              setInitSelectedProductID={setSelectedProductID}
            />
          ) : (
            <ProductBlock
              {...props}
