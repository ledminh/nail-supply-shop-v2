...
type Props = {
  ...
  setProductModalType: React.Dispatch<
...
function useEditProduct({
  ...
  products,
  setProductModalType,
  setBeingEditedProduct,
  setIsProductModalOpen,
  setIsProductGroupModalOpen,
  beingEditedProductGroup,
}: Props) {
  const updateProduct = (
    id: string,
    serialNumber: string,
    ...
    processImages(images)
      ...
      .then((data) => {
        setProducts(
          products.map((prod) => (prod.id === data.id ? data : prod))
        );
      });
  };
  ...
  const onEditProduct = (prodID: string) => {
    ...
    if (beingEditedProductGroup) {
      ...
      setBeingEditedProduct(
        beingEditedProductGroup.products.find(
          (prod) => prod.id === prodID
        ) as Product
      );
    } else {
      setBeingEditedProduct(
        products.find((prod) => prod.id === prodID) as Product
      );
    }
