        ...
        switch(type) {
            case 'create':
                ...
                form.parse(req, (err, fields) => {
                    ...
                    if(typeof name !== "string" || typeof description !== "string" || typeof imageFileName !== "string") {
...
function createCategory({name, description, imageFileName}:CreateCategoryProps, res: NextApiCategoryResponse) {
    
    DB.createCategory({name, description, imageFileName}).then((dbRes) => {
        if(!dbRes.success) {
            res.status(500).json({ success: false, message: dbRes.message });
            return;
        }
